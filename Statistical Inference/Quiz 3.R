...
and a standard deviation of 30cc. What is a 95% Students T confidence interval for the mean brain 
...
Answer: qt(0.975, df=8)*30/3 = [1077,1123]
...
and the standard system on the remaining 10 nights. They calculated the nightly median waiting time (MWT) to see a physician. 
The average MWT for the new system was 3 hours with a variance of 0.60 while the average MWT for the old system was 5 hours with 
a variance of 0.68. Consider the 95% confidence interval estimate for the differences of the mean MWT associated with the new system. 
...
(3-5) + c(-1,1) * qt(0.975, df = 18) * sp * (1/10 + 1/10)^0.5 

Answer: Not [-2,70, -1.29]
...
waiting time (MWT) to see a physician. The average MWT for the new system was 4 hours with a standard deviation 
of 0.5 hours while the average MWT for the old system was 6 hours with a standard deviation of 2 hours. 
...
Answer: 
sp <- sqrt((99* 0.5^2 + 099 * 2^2 )/ (100+100-2))        
(4-6) + c(-1,1) * qt(0.95, df = 18) * sp * (1/100 + 1/100)^0.5 
...
The average difference from follow-up to the baseline (followup - baseline) was −3 kg/m2 for the treated group and 1 kg/m2 for the
placebo group. The corresponding standard deviations of the differences was 1.5 kg/m2 for the treatment group and 1.8 kg/m2 for the 
placebo group. Does the change in BMI over the four week period appear to differ between the treated and placebo groups? 
...
(-3-1) + c(-1,1) * qt(0.95, df = 16) * sp * (1/9 + 1/9)^0.5 
